...
%load('Example4_Result_con1_200_temp1.mat');
times = 1;
datasrd = load('datasrdrealfull.tab');
%beta = load('beta.tab')
n = size(datasrd, 1); p = size(datasrd, 2) - 1;
d = 2;
...
init = zeros(p,2,times);
mave = zeros(p,2,times);
sr = zeros(p,2,times);
best = zeros(p,2,times);
varbest = zeros((p-d) * 2,(p-d)* 2,times);
grad = zeros(p^2, p^2, times);
eflag = []; 
beta = [1,0,7.866856e-01,3.694695e-01,4.419761e-01,5.965604e-01;0,1,-1.113351e-01,1.190171e-01,1.953554e+00,2.142364e-01]'

for ii = 1:times
    ii
    [ init(:,:,ii), best(:,:,ii), grad(:, :, ii), varbest(:, :, ii)] = SIRMIM(n,p, ii, datasrd, beta, beta, 0);
    rcorr(ii,:), pdist(ii,:),xcorr(ii,:)
end

%best1 = median(best(:, :, 1:100), 3);
%wgrad = mean(grad(:, :, 1:100), 3);
%for ii = 1:times
 %   ii
  %  [rcorr(ii,:), pdist(ii,:), ...
%xcorr(ii,:), init(:,:,ii),mave(:,:,ii),...
%			sr(:,:,ii),best(:,:,ii), grad(:, :, ii), varbest(:, :, ii)] = SIRMIM(n,p, ii, datasrd, beta, best1, 1);
 %   rcorr(ii,:), pdist(ii,:),xcorr(ii,:)
%end
save Example4_real.mat varbest best n times init

      function [ init,  best, grad, varbest] = SIRMIM(n,p, ii, datasrd, beta, best, flag)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                         %
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
dim = 2
      x = datasrd((((ii - 1) * n + 1):(ii *  n)), 2:end);

y = datasrd((((ii - 1) * n + 1):(ii *  n)), 1);
...
invS = eye(p^2);
if(flag == 1)
grad = gobjcon(best, invfest, z, p, n);
invS = inv(grad);
end


tic
lb = [ 0.3, 0.3, 0.3, 0.3;  0.3,  -0.5, 0.3, -0.5];
up = [ 0.5, 0.5, 0.5, 0.5; 0.5, -0.3, 0.5, -0.3];
lb(:) = -1;
up(:) = 1;
...
[best,ee,eflag, output, lambda, grad, hessian] = fmincon(@(best) objcon(best,invfest,z,p,n, beta, invS),beta(3:6, :),...
    [],[],[],[],[], [], [],optimset('LargeScale','on', 'TolCon',1e-8,'TolFun',1e-8,'algorithm','active-set', 'MaxIter', 10000, 'MaxFunEvals', 10000));
toc
best = [beta(1:2, 1:2); best]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                         %
%                           BACK-TRANSFORMATION                           %
%                                                                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
best = ss^(-1/2)*best;
init = init * inv(init(1:dim, 1:dim)) * beta(1:dim, 1:dim);
init = ss^(-1/2)*init
best2 = [best(3:6, 1); best(3:6, 2)];
grad = gobjcon(best, invfest, z, p, n);
end
%invS =  inv(grad);
 hessian = numjacobian(best2, invfest, z, p, dim, n, beta);
		    invgrad = (hessian' * invS * hessian)^(-1) * hessian'* invS;

			       varbest = invgrad * (grad)  * invgrad';%
...
% The objective function for fmincon
	 function ee = objcon(best,invfest,z,p,n, beta, wgt)
best = [beta(1:2, 1:2); best];
...
function ee = objcon1(best2,invfest,z,p,d, n, beta)
best = [beta(1:d, 1:d); best2];
zfest = kernelregression(z*best,z,z*best, 'kernel','epanechinikov');
zres = z - zfest;
zfest = kernelregression(z*best,invfest,z*best, 'kernel','epanechinikov');
resinv = invfest - zfest;
...
temp =  innersum(:, :, i);
innersum2(:, :, i) = temp(:) * temp(:)';
end
ee = mean(innersum, 3);
ee2 = mean(innersum2, 3);
wgt = inv(ee2);
ee = ee(:)' * wgt * ee(:);
...
%						  ee = diag(smean(:));
	 ...
	 function ee = hobjcon(best2,invfest,z,p,d, n, beta)
n = size(z,1);
...
function ee = hobjcon2(best2,invfest,z,p,d, n, beta)
n = size(z,1);
best2 = reshape(best2, (p-2), d);
best = [beta(1:d, 1:d); best2];
zfest = kernelregression(z*best,z,z*best, 'kernel','epanechinikov');
...
function J=numjacobian(x, invfest, z, p, d, n, beta)
n = size(z, 1)

	     nn=length(x); % n=2 in your case, but can be higher 
	     fx=hobjcon2(x, invfest, z, p, d, n, beta); % evaluate the function at point x
...
size(fx)
   ...
   % numerically using first order forward difference approximation
	     xstep(i)=x(i)+step;
	     J(:,i)=(hobjcon2(xstep, invfest, z, p, d, n, beta)-fx)/step;
	     end
