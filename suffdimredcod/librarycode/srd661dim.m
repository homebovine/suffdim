...
function [rcorr, pdist, xcorr, init, mave, sr, best] = Example66
times = 1;
...
for ii = 1:times
    [ init(:, :, ii),  cross(ii)] = SAVEMIM(n,p, ii, datasrd);
    
    
end
median(cross)
save Example6_real.mat  init cross

      function [init,  cross] = SAVEMIM(n,p, ii, datasrd)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                         %
%           THE SAVE METHOD FOR MULTIPLE INDICES MODEL                    %
%                                                                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%                                                                         %
%                                                                         %
%                 SLICED AVERRAGE VARIANCE ESTIMATON                      %
%                                                                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%2B[m, m0] = reg_local_linear_smoothing(cuvx, cuvy, 2, 2, cuvnewx)
%cross = median((m - y0).^2); 
%cuvx =  x0 * init;
%cuvy = y0;
%cuvnewx = newx * init;
%fity = fit([cuvx(:, 1)],  cuvy, 'linearinterp');
%fity = feval(fity, [cuvnewx(:, 1)]);
%cross = median((fity - newy).^2);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                         %
%                        RESULTS SUMMARY                                  %
...
function ee = objcon(best,blam11,blam22,z,p,zz, beta, wgt)
n = size(z,1);
d = size(beta, 2);
best = [beta(1:d); best];
...
eematrix = blam11'*zres/size(z,1) - mean(blam33,3);
ee = (eematrix(:))'*wgt *(eematrix(:))/2;



function ee = gobjcon(best,blam11,blam22,z,p,zz)
n = size(z,1);
zfest = kernelregression(z*best,z,z*best,'kernel','epanechinikov');
zres = z - zfest;
invsqfesttemp=kernelregression(z*best,zz,z*best,'kernel','epanechinikov');
	 %eematrix = zeros(p * p, p * p, n);
for ii = 1:p
    for jj = 1:ii
        invsqfest = invsqfesttemp(:,(ii-1)*p+jj);
        blam(ii,jj,:) = invsqfest - zfest(:,ii).*zfest(:,jj);
...
for ii = 1:n
    blam33(:,:,ii) = blam22(:,:,ii) * blam(:,:,ii);
tempee  = (blam11(ii, :)' * zres(ii, :) - blam33(:, :, ii));
eematrix(:, :, ii) = tempee(:) * tempee(:)'; 
eematrix2(:,  ii) = tempee(:); 
end
	   ee2 = mean(eematrix2, 2);
	   ee = mean(eematrix, 3);%-  ee2(:) * ee2(:)' ;
;
...
function ee = hobjcon(best2,blam11,blam22,z,p, d, zz, beta)
n = size(z,1);
...
%mblam33 = mean(blam33, 3)
%for ii = 1:n
%tempee  = (blam11(ii, :)' * zres(ii, :) - mblam33);
%eematrix(:, :, ii) = tempee;
%end
	   ee = mean(eematrix, 3);
ee = ee(:);

	   function ee = hobjcon2(best2,blam11,blam22,z,p, d, zz, beta)
n = size(z,1);
best2 = reshape(best2, (p-d), d);
best = [beta(1:d); best2];
zfest = kernelregression(z*best,z,z*best,'kernel','epanechinikov');
...
function ee = objcon1(best2,blam11,blam22,z,p,  zz, beta, wgt)
d = size(beta, 2);
n = size(z,1);
best2 = reshape(best2, (p-d), d);
best = [beta(1:d); best2];
...
for ii = 1:n
    blam33(:,:,ii) = blam22(:,:,ii) * blam(:,:,ii);
tempee  = (blam11(ii, :)' * zres(ii, :) - blam33(:, :, ii));
	 tempee2 = tempee(:)' * tempee(:);
eematrix(:, :, ii) = tempee;
ee2matrix(:, :, ii) = tempee2;
end
ee = mean(eematrix, 3);
wgt = inv(mean(ee2matrix, 3));% - ee(:)' * ee(:));
ee = ee(:)' * wgt * ee(:);
	 
function J=numjacobian(x, blam11,blam22,z,p, d, zz, beta)
n = size(z, 1)

	     nn=length(x); % n=2 in your case, but can be higher 
	     fx=hobjcon2(x, blam11,blam22,z,p, d, zz, beta); % evaluate the function at point x
...
xstepl = x;
   ...
   % numerically using first order forward difference approximation
	     xstep(i)=x(i)+step;
xstepl(i)=x(i)-step;
	     %fx=hobjcon(xstepl, blam11,blam22,z,p, d, zz, beta); % evaluate the function at point x
	     J(:,i)=(hobjcon2(xstep, blam11,blam22,z,p, d, zz, beta)-fx)/( step);
	     end
...
if (isempty(u)),
   ...
   if sig>0
      u1 = sig * (4/(3))^(1/5) * n^(-1/5);
      u2 = sig * (4/(3))^(1/5) * n^(-1/8);
      u1(end, :) = u2(end, :);
      u = u1;
   else
      u = ones(1,p);
