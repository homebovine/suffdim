function [rcorr, pdist, xcorr, init, mave, sr, best] = Example33
%load('Example3_Result_con1_200_norm_temp.mat')
...
for ii = 1:times
    ii
    [ init(:, :, ii), cross(ii)] = PHDMIM(n,p, ii, datasrd);
...
%			sr(:,:,ii),best(:,:,ii), grad(:, :, ii), varbest(:, :, ii)] = PHDMIM(n,p, ii, datasrd, beta, beta1, 1);
%    rcorr(ii,:), pdist(ii,:),xcorr(ii,:)
%end

save Example3_real.mat init cross

      function [ init,  cross] = PHDMIM(n,p, ii, datasrd)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                         %
%           THE PHD METHOD FOR MULTIPLE INDICES MODEL                     %
%                                                                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                         %
...
%                                                                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
bLam = (repmat(y,1,p).*z)'*z/n - mean(y)*eye(p);
[U,S,V] = svd(bLam); 
init = U(:,1:dim);
for j=1:dim
    i=1;
...
%2Bcuvy = y0;
%cuvnewx = newx * init;
%fity = fit([cuvx(:, 1), cuvx(:, 2)],  cuvy, 'lowess');
...
function ee = objcon(best,z,y,zz,n, beta, wgt)
best = [beta(1:2, 1:2); best];
zfest = kernelregression(z*best,zz,z*best,'kernel','epanechinikov');
zzres = zz - zfest;
fest = kernelregression(z*best,y,z*best,'kernel','epanechinikov');
ee = ((zzres'*(y - fest))/n)'*wgt * ((zzres'*(y - fest))/n)/2;

function ee = gobjcon(best, z,y,zz,n)
p2  =  size(zz, 2)
zfest = kernelregression(z*best,zz,z*best,'kernel','epanechinikov');
zzres = zz - zfest;
fest = kernelregression(z*best,y,z*best,'kernel','epanechinikov');
innersum = zeros(p2, p2, n);
for i = 1:n
	  innersum(:, :, i)  = (zzres(i, :) * (y(i) - fest(i)))' * (zzres(i, :) * (y(i) - fest(i)));
innersum2(i, :) =  (zzres(i, :) * (y(i) - fest(i)));
end
ee2 = mean(innersum2);
ee = mean(innersum, 3);% - ee2(:) * ee2(:)' ;

function ee = objcon1(best2, z,y,zz,p, d, n, beta)
...
innersum = zeros(n, p2);
for i = 1:n
	  innersum(i, :)  = (zzres(i, :) * (y(i) - fest(i))) ;
	  innersum2(:, :, i) = (zzres(i, :) * (y(i) - fest(i)))' * (zzres(i, :) * (y(i) - fest(i)));
end

ee = mean(innersum);
wgt = pinv(mean(innersum2,3) );% - ee(:) * ee(:)'
ee = ee(:)' * wgt * ee(:);



function ee = hobjcon(best2, z,y,zz,p, d, n, beta)
...
function ee = hobjcon2(best2, z,y,zz,p, d, n, beta)
best2 = reshape(best2, (p-2), d);
best = [beta(1:d, 1:d); best2];
p2  =  size(zz, 2);
zfest = kernelregression(z*best,zz,z*best,'kernel','epanechinikov');
...
ee = ((zzres'*(y - fest))/n);
				...
				function J=numjacobian(x,  z, y, zz, p, d, n, beta)

	     nn=length(x); % n=2 in your case, but can be higher 
		fx=hobjcon2(x, z, y, zz,p, d,  n, beta); % evaluate the function at point x
...
%				fx=hobjcon(xstepl, z, y, zz,p, d,  n, beta); % evaluate the function at point x
				J(:,i)=(hobjcon2(xstep, z, y, zz, p, d, n, beta)-fx)/(step);
	     end
...
% the constraint function for gfmincon
function [c,ceq]= gconstraint(best, resbest)
[p,d] = size(best);
