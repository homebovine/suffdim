function [rcorr, pdist, xcorr, init, mave, sr, best] = Example77
times = 1;
datasrd = load('datasrdrealfull.tab');
times = 1

n = size(datasrd, 1); %(size(datasrd, 1) + 1)/2;
p = size(datasrd, 2) - 1;
...
for ii = 1:times
    ii
    [ init(:,:,  ii), cross(ii)] = DRMIM(n,p, ii, datasrd);
...
%for ii = 1:times
 %   ii
  %  [rcorr(ii,:), pdist(ii,:), ...
   %     xcorr(ii,:), init(:,:,ii),mave(:,:,ii),...
%			sr(:,:,ii),best(:,:,ii), grad(:, :, ii), varbest(:, :, ii), eflag(ii)] = DRMIM(n,p, ii, datasrd, beta, wgrad, 1);
 %   rcorr(ii,:), pdist(ii,:),xcorr(ii,:)
%end

save Example7_real.mat  init cross

      function [init, cross] = DRMIM(n,p, ii, datasrd)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                         %
%             THE DR METHOD FOR MULTIPLE INDICES MODEL                    %
%                                                                         %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
dim = 1
%y0 = datasrd(((ii - 1) * n + 1): (ii * n), 1);
%x0 = datasrd(((ii - 1) * n + 1): (ii * n), 2:7);
%newy = datasrd(((ii - 1) * n + 1): (ii * n), 8);
%newx = datasrd(((ii - 1) * n + 1): (ii * n), 9:end);
%      x = x0;
...
x = x0;
y = y0;
%beta(1:p,1) = 1; beta([1:2:p],2)=1;beta([2:2:p],2)= -1;
%beta(1:dim, 1:dim) = eye(dim)
...
for ii = 1:p
    for jj = 1:ii
        invsqfest(ii,jj,:) = kernelregression(y,z(:,ii).*z(:,jj),y,...
            'kernel','epanechinikov');
        invsqfest(jj,ii,:) = invsqfest(ii,jj,:);
    end
end
blam22 = (invfest'*invfest/n);
for ii = 1:n
    blam11(ii,:) = ((eye(p)-invsqfest(:,:,ii))*x(ii,:)')';
end
bLam = -(blam11'*x/n) + blam22*blam22' + trace(blam22)*blam22;
[U,S,V] = svd(bLam);
init = U(:,1:dim); 
for j=1:dim
    i = 1;
...
init = ss^(-1/2)*init; 
init = init * inv(init(1:dim, 1:dim))
allOneString = sprintf('%d,' ,  init)
cuvx =  x0 * init;
cuvy = y0;
cuvnewx = newx * init;
fity = csaps(cuvx, cuvy, 'xx',   cuvnewx);
cross = median((fity - newy).^2); 
%cuvx =  x0 * init;
%cuvy = y0;
%cuvnewx = newx * init;
%fity = fit(cuvx,  cuvy, 'smoothingspline');
%fity = feval(fity, cuvnewx)
%cross = median((fity - newy).^2);
%reg_local_linear_smoothing(cuvx, cuvy, 2, )
[m, m0] = reg_local_linear_smoothing(cuvx, cuvy, 2, 1, cuvnewx)
cross = mean((m0 - newy).^2); 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                         %
%                        RESULTS SUMMARY                                  %
%                                                                         %
...
% The objective function for fmincon
	 function ee = objcon(best,blam11,blam22,invsqfest,invfest,z,p, beta, wgt)
d = size(best, 2);
	 best = [beta(1:d); best];
n = size(z,1);
...
eematrix = -mean(blam55,3) + blam22*(invfest'*zres/n) + ...
    trace(blam22)*(invfest'*zres/n);
ee = (eematrix(:))'* wgt *(eematrix(:));


function ee = objcon1(best2, blam11,blam22,invsqfest,invfest,z,p, d, beta)
n = size(z,1);
best = [beta(1:d); best2];
zfest = kernelregression(z*best,z,z*best,'kernel','epanechinikov');
zres = z - zfest;
...
%temp =  blam22*(invfest'*zres/n) + ...
 %   trace(blam22)*(invfest'*zres/n); 
for ii = 1:n
	   blam55(:,:,ii) = - (eye(p)-invsqfest(:,:,ii))*blam44(:,:,ii) + blam22 * invfest(ii, :)' * zres(ii, :) + trace(blam22) * invfest(ii, :)' * zres(ii, :);
temp = blam55(:, :, ii);
   mblam55(:,:,ii) = temp(:) * temp(:)';

end
ee2matrix = mean(mblam55, 3);
wgt = inv(ee2matrix);
eematrix = mean(blam55, 3);
ee = eematrix(:)' * wgt * eematrix(:);


function ee = gobjcon(best,blam11,blam22,invsqfest,invfest,z,p)
n = size(z,1);
zfest = kernelregression(z*best,z,z*best,'kernel','epanechinikov');
zres = z - zfest;
zz = [];
...
mblam55 = zeros(n, p * p);
for ii = 1:n
	   blam55temp = - (eye(p)-invsqfest(:,:,ii))*blam44(:,:,ii) + blam22 * invfest(ii, :)' * zres(ii, :) + trace(blam22) * invfest(ii, :)' * zres(ii, :);

blam55(:, :, ii) = blam55temp(:) * blam55temp(:)';
		    mblam55(ii, :) = blam55temp(:);
end
 mmblam55 = mean(mblam55);
 ee = mean(blam55, 3) ;


function ee = hobjcon(best2, blam11,blam22,invsqfest,invfest,z,p, d, beta)
n = size(z,1);
best2 = reshape(best2, (p - d), d);
best = [beta(1:d); best2];
zfest = kernelregression(z*best,z,z*best,'kernel','epanechinikov');
zres = z - zfest;
zz = [];
...
function ee = hobjcon2(best2, blam11,blam22,invsqfest,invfest,z,p, d, beta)
	 n = size(z,1);
best2 = reshape(best2, (p - d), d);
best = [beta(1:d); best2];
n = size(z,1);
...
function J=numjacobian(x, blam11,blam22,invsqfest,invfest,z,p, d, beta)
n = size(z, 1)

	     nn=length(x); % n=2 in your case, but can be higher 
	     fx=hobjcon2(x, blam11,blam22,invsqfest,invfest,z,p, d, beta); % evaluate the function at point x
...
xstepl = x;
   ...
   % numerically using first order forward difference approximation
	     xstep(i)=x(i)+step;
xstepl(i)=x(i)-step;
%fx=hobjcon(xstepl, blam11,blam22,invsqfest,invfest,z,p, d, beta); % evaluate the function at point x
	     J(:,i)=(hobjcon2(xstep, blam11,blam22,invsqfest,invfest,z,p, d, beta)-fx)/(step);
	     end


% the constraint function for fmincon
function [c,ceq]=constraint(best, beta)
[p,d] = size(best);
ceq = best'*best-beta' * beta;
c = zeros(d,1);
for j = 1:d
    i = 1;
...
[p,d] = size(best);
beq = zeros(p, d);
		    beq(1:d, 1:d) = diag([1/sqrt(6), -1/sqrt(6)]);%eye(d) * 0.5;
Aeq = zeros(p, p);
Aeq(1:d, 1:d) = eye(d);
ceq = Aeq * best - beq;
c = 0;
