...
for ii = 1:times
    ii
    [ init(:, :, ii), cross(ii)] = SIRMIM(n,p, ii, datasrd);
allOneString = sprintf('%d,' ,  init(:, :, ii))    
...
%end
save Example4_real.mat  init cross

      function [ init, cross] = SIRMIM(n,p, ii, datasrd)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                                         %
...
%cross = median((fity - newy).^2);
sig = mad(y, 1) * 1.4826;%median(abs(y-repmat(med,n,1))) / 0.6745;
h = sig * (4/(3*n))^(1/6)
